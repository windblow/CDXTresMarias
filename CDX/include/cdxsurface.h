...
public:
	...
	HRESULT Create(CDXScreen* pScreen, const TCHAR *szFilename, BYTE memoryType = CDXMEM_VIDTHENSYS);
	...
	HRESULT DrawText(LPCTSTR pString, COLORREF col, LPRECT pRect);
    HRESULT SaveAsBMP(LPCTSTR szFilename);
	...
	HRESULT     ChangeFont(LPCWSTR FontName, int Width, int Height, int Attributes = FW_NORMAL);
    ...
    HRESULT     ReLoadBitmap(TCHAR * filename = NULL);
    void        FunctionMapper(void);
    void        SetCaptureFileName(LPCWSTR name) { _tcsncpy_s(m_CaptureFileName, name, 256); }
	TCHAR*      GetCaptureFileName(void) { return m_CaptureFileName; }
...
private:  // Private member data (all data should be private)
	...
	TCHAR       m_FileName[256];		// The name of the bitmap file loaded, used in Restore
    TCHAR       m_CaptureFileName[256];
	...
	TCHAR       m_FontName[256];        // Current font associated to the surface
