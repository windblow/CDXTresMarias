...
//////////////////////////////////////////////////////////////////////////////////
BOOL CDXMusic::Play(const TCHAR *Filename)
{
	if(Filename == NULL) return FALSE;
	TCHAR buffer[256];

	_stprintf(buffer, _T("open %s type sequencer alias MUSIC"), Filename);
	...
	if (mciSendString(_T("play MUSIC from 0 notify"), NULL, 0, m_hWnd) != 0)
	{
		return(FALSE);
	}
...
BOOL CDXMusic::Pause(void)
{
	...
	if (mciSendString(_T("stop MUSIC"), NULL, 0, NULL) != 0)
	{
		return(FALSE);
	}
...
{
	if (mciSendString(_T("play MUSIC notify"), NULL, 0, m_hWnd) != 0)
	{
		return(FALSE);
	}
