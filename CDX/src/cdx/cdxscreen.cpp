...
HRESULT CDXScreen::Flip(BOOL VSync, BOOL FlipWithStretch, BOOL displayFPS)
{
	...
	if(m_bFullScreen)
	{
		CDXLOG2(_T("do a fullscreen flip"));
...
//
//////////////////////////////////////////////////////////////////////////////////
HRESULT CDXScreen::LoadBitmap(const TCHAR* szFilename)
{
    HRESULT rval;

	CDXLOG(_T("START: CDXScreen::LoadBitmap(szFilename: %s)"), szFilename);

	if(szFilename == NULL) 
        return -1;

	CDXLOG(_T("load the bitmap into the back buffer"));

	rval = DDReLoadBitmap(GetBack()->GetDDS(), szFilename);

	CDXLOG(_T("END: CDXScreen::LoadBitmap"));
...
//
//////////////////////////////////////////////////////////////////////////////////
HRESULT CDXScreen::LoadPalette(const TCHAR* szFilename)
{
	HRESULT rval = S_OK;
	...
	CDXLOG(_T("Load the palette"));

	rval = DDLoadPalette(m_lpDD, &m_lpDDPalette, szFilename);
	if (FAILED(rval))
		return rval;

	CDXLOG(_T("Assign the palette to the front surface"));
	GetFront()->GetDDS()->SetPalette(m_lpDDPalette);
...
{
    HRESULT rval = 0;
	CDXLOG(_T("START: CDXScreen::Restore"));
	...
	CDXLOG(_T("END: CDXScreen::Restore"));
...
HRESULT CDXScreen::SetColor(int col, int r, int g, int b)
{
	...
	CDXLOG(_T("START: CDXScreen::SetColor(col:%i, r:%i, g:%i, b:%i)"), col, r, g, b);
	CDXLOG(_T("Get the color entry"));
	...
	CDXLOG(_T("Set the color entry"));

	rval = m_lpDDPalette->SetEntries(0, col, 1, pe);

	CDXLOG(_T("END: CDXScreen::SetColor"));
...
HRESULT CDXScreen::GetColor(int col, int *r, int *g, int *b)
{
	...
	CDXLOG(_T("START: CDXScreen::GetColor(col:%i, r:%i, g:%i, b:%i)"), col, r, g, b);
	CDXLOG(_T("Get the color entry"));
	...
	*b = pe[0].peBlue;
	...
	CDXLOG(_T("r:%i, g:%i, b:%i)"), pe[0].peRed, pe[0].peGreen, pe[0].peBlue);
	CDXLOG(_T("END: CDXScreen::GetColor"));
...
HRESULT CDXScreen::SetPalette(int Start, int Count, LPPALETTEENTRY lpPE)
{
	...
	CDXLOG(_T("START: CDXScreen::SetPalette(start:%i, count:%i, lpPE:%i)"), Start, Count, lpPE);

	rval = m_lpDDPalette->SetEntries(0, Start, Count, lpPE);

	CDXLOG(_T("END: CDXScreen::SetPalette"));
...
HRESULT CDXScreen::GetPalette(int Start, int Count, LPPALETTEENTRY lpPE)
{
	...
	CDXLOG(_T("START: CDXScreen::GetPalette(start:%i, count:%i, lpPE:%i)"), Start, Count, lpPE);

	rval = m_lpDDPalette->GetEntries(0, Start, Count, lpPE);

	CDXLOG(_T("END: CDXScreen::GetPalette"));
...
HRESULT CDXScreen::FillPalette(int r, int g, int b)
{
	...
	CDXLOG(_T("START: CDXScreen::FillPalette(r:%i, g:%i, b:%i)"), r, g, b);
	CDXLOG(_T("Get the color entries"));

	rval = m_lpDDPalette->GetEntries(0, 0, 256, pe);
    if(FAILED(rval) )
        return rval;

	CDXLOG(_T("Fill the color entries"));
	...
	CDXLOG(_T("Set the color entries"));
	rval = m_lpDDPalette->SetEntries(0, 0, 256, pe);

	CDXLOG(_T("END: CDXScreen::FillPalette"));
...
HRESULT CDXScreen::GreyScale(void)
{
	...
	CDXLOG(_T("START: CDXScreen::GreyScale"));
	CDXLOG(_T("Get the color entries"));
	...
	CDXLOG(_T("END: CDXScreen::GreyScale"));
...
HRESULT CDXScreen::EnumDrivers( void )
{
    ...
    rval = DirectDrawEnumerateEx( (LPDDENUMCALLBACKEX) DDEnumCallback, (LPVOID)m_Drivers, DDENUM_ATTACHEDSECONDARYDEVICES | DDENUM_NONDISPLAYDEVICES );
#else
...
HRESULT CDXScreen::ReloadAllSurfaces(void)
{
    ...
    for(pSurface = m_SurfaceList.GetFirst(); pSurface != NULL; pSurface = m_SurfaceList.Next(pSurface))
    {
        ...
        // Delete the file.
        _tremove(pSurface->GetCaptureFileName());
	}
...
HRESULT CDXScreen::SaveAllSurfaces(void)
{
    ...
    {
        // Create the filename
        _stprintf(filename,_T("surface%04d.bmp"), pSurface->GetSurfaceId());
