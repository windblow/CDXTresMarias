...
CDXMidi::CDXMidi(void)
{
	...
	{
#if defined NTDX3 || defined NODIRECTMUSIC
...
HRESULT CDXMidi::Shutdown()
{
	...
	if( (rval = mciSendString(_T("close all"), NULL, 0, NULL)) != 0)
		return -1;
...
//////////////////////////////////////////////////////////////////////////////////
int CDXMidi::LoadMidi(const TCHAR *name)
{
    int id = -1;
	int index;
    ...
    m_Midi[id].fileName = new TCHAR[256];
	_tcsncpy(m_Midi[id].fileName, name, 256);
...
HRESULT CDXMidi::Play(int MidiID)
{
	...
	_stprintf(buffer, _T("open %s type sequencer alias MUSIC"), m_Midi[MidiID].fileName);
	...
	if ( (rval = mciSendString(_T("play MUSIC from 0 notify"), NULL, 0, m_hWnd) ) != 0)
		return -4;
